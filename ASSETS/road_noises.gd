...
func _physics_process(delta):
	...
	$rolling.volume_db = clamp(linear_to_db(max(total_rolling/1000.0,0.0)*0.75),-80.0,0.0)
	$rolling.max_db = $rolling.volume_db

	$rub.volume_db = clamp(linear_to_db((max(total_rubbing-total_skidding,0.0) *0.75)/300.0),-80.0,0.0)
	$rub.max_db = $rub.volume_db

	$skid.volume_db = clamp(linear_to_db(max(total_skidding,0.0)/30.0 *0.75),-80.0,0.0)
