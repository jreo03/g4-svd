...
func predict_impulse(p_position: Vector3, p_impulse: Vector3) -> void:
	...
	var COM: Vector3 = PSDB.center_of_mass*global_transform.basis
	
	plin += p_impulse * _inv_mass
	pain += _inv_inertia_tensor*((p_position - COM).cross(p_impulse))
...
func _integrate_forces(state: PhysicsDirectBodyState3D):
	...
	for wheel in wheels:
		...
		var od_red: float = max(abs(align_t/rads2rpm) -w_torque_est/rads2rpm,0.0)
		...
		if wheel.df_lockto:
			...
			var diff_dist: float = max(diff_wheel_travel_distance -locking_torque,0.0)
			if diff_wheel_travel_distance<0:
				diff_dist = min(diff_wheel_travel_distance +locking_torque,0.0)
			...
			var awd_diff_dist: float = max(awd_diff_wheel_travel_distance -awd_locking_torque,0.0)
			if awd_diff_wheel_travel_distance<0:
				awd_diff_dist = min(awd_diff_wheel_travel_distance +awd_locking_torque,0.0)
