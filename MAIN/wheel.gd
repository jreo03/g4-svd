...
@export_range(0,1) var DIFF_Locking_Power: float
@export_range(0,1) var DIFF_Locking_Coast: float
...
func _physics_process(delta):
	...
	if is_colliding():
		...
		var ppatch_velocity: Vector3 = ppatch_global_velocity*c_axis.orthonormalized()
		...
		var compressed: float = max(SUS_rest_length - HUB_pos,0.0)
		...
		compressed = max(SUS_rest_length - HUB_pos,0.0)
